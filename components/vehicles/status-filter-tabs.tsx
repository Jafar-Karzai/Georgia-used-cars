    ...
    <div className="bg-brand-red-600 rounded-lg p-6 shadow-lg">
      <Tabs value={currentGroup} onValueChange={handleTabChange} className={className}>
        <TabsList className="grid w-full grid-cols-3 h-auto p-1.5 bg-white/10 backdrop-blur-sm border border-white/20">
          <TabsTrigger
            value="all"
            className="data-[state=active]:bg-white data-[state=active]:text-precision-900 data-[state=inactive]:text-white data-[state=inactive]:hover:bg-white/20 py-3 px-4 gap-2 flex items-center justify-center transition-all group"
          >
            <Car className="h-4 w-4" aria-hidden="true" />
            <span className="font-semibold">All Vehicles</span>
            {counts && (
              <Badge variant="secondary" className="ml-1 h-5 px-1.5 text-xs font-bold group-data-[state=active]:bg-precision-100 group-data-[state=active]:text-precision-900 group-data-[state=inactive]:bg-precision-900 group-data-[state=inactive]:text-white">
                {counts.all}
              </Badge>
            )}
          </TabsTrigger>
          ...
          >
            ...
            {counts && (
              <Badge variant="secondary" className="ml-1 h-5 px-1.5 text-xs font-bold group-data-[state=active]:bg-emerald-100 group-data-[state=active]:text-emerald-900 group-data-[state=inactive]:bg-precision-900 group-data-[state=inactive]:text-white">
                {counts.arrived}
              </Badge>
            )}
          </TabsTrigger>
